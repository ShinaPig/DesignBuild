 ...
 * @author ziyue ji
 */
public class CComparePanel extends JPanel {
    ...
    public CComparePanel(int companyid) {
    ...
    private void searchbuttonActionPerformed(ActionEvent e) {
        String sql=null;
        sql="SELECT count(*) FROM data,device WHERE data.deviceid=device.deviceid and data.deviceid=? AND time>=? AND time<=? AND value<? and companyid="+companyid;
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                ps.setTimestamp(2, new Timestamp(((java.util.Date)spinner3.getValue()).getTime()));
                ps.setTimestamp(3, new Timestamp(((java.util.Date)spinner4.getValue()).getTime()));
