 ...
 * @author ziyue ji
 */
public class CDetailPanel extends JPanel {
    ...
    public CDetailPanel(int companyid) {
    ...
    public Object[][] queryUserData(List<Data> list) {
//        java.util.List<User> list=this.queryAllUser();
    ...
    public List<Data> queryAllUser(){
        String sql="select dataid,dataname,value,time,data.deviceid from data,device where data.deviceid=device.deviceid and companyid="+companyid;
    ...
    private void searchbuttonActionPerformed(ActionEvent e) {
        String sql=null;
        if(searchbox.getSelectedIndex()==0)sql="SELECT * FROM data,device WHERE data.deviceid=? AND time>=? AND time<=? AND data.deviceid=device.deviceid and companyid="+companyid;
        if(searchbox.getSelectedIndex()==1)sql="SELECT dataid,dataname,value,time,data.deviceid FROM data,category,device WHERE device.categoryid=category.categoryid AND data.deviceid=device.deviceid AND device.categoryid=? AND time>=? AND time<=? and companyid="+companyid;
        if(searchbox.getSelectedIndex()==2)sql="SELECT dataid,dataname,value,time,data.deviceid FROM data,family,device WHERE device.familyid=family.familyid AND data.deviceid=device.deviceid AND device.familyid=? AND time>=? AND time<=? and companyid="+companyid;
        List<Data> list=new ArrayList<>();
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
