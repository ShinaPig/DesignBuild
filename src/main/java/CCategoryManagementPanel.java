 ...
 * @author ziyue ji
 */
public class CCategoryManagementPanel extends JPanel {
    ...
    public CCategoryManagementPanel(int companyid) {
...
//        java.util.List<User> list=this.queryAllUser();
    ...
    public List<Category> queryAllUser(){
        String sql="SELECT DISTINCT category.categoryid,categoryname FROM category,device WHERE device.categoryid=category.categoryid AND device.companyid="+companyid;
    ...
    private void searchbuttonActionPerformed(ActionEvent e) {
        String sql=null;
        if(searchbox.getSelectedIndex()==0)sql="SELECT DISTINCT category.categoryid,categoryname FROM category,device WHERE category.categoryid=? AND device.categoryid=category.categoryid AND device.companyid="+companyid;
        if(searchbox.getSelectedIndex()==1)sql="SELECT DISTINCT category.categoryid,categoryname FROM category,device WHERE categoryname=? AND device.categoryid=category.categoryid AND device.companyid="+companyid;
        List<Category> list=new ArrayList<>();
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    private void button2ActionPerformed(ActionEvent e) {
        String sql=null;
        sql="SELECT DISTINCT category.categoryid,categoryname FROM category,device WHERE device.categoryid=category.categoryid AND device.companyid="+companyid;
        List<Category> list=new ArrayList<>();
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                try (ResultSet rs = ps.executeQuery()) {
...
//            List<User> list = new ArrayList<>();
            try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASSWORD)) {
                sql = "DELETE data FROM data, device WHERE device.categoryid=? AND data.deviceid=device.deviceid AND device.companyid="+companyid;
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
...
//                    JOptionPane.showMessageDialog(null, "Categoryid cannot be found!", "Error!",JOptionPane.ERROR_MESSAGE);
                ...
                sql = "DELETE device FROM device WHERE device.categoryid=? AND device.companyid="+companyid;
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ps.setInt(1, Integer.parseInt(textField2.getText().trim()));
                    ps.executeUpdate();
                } catch (SQLException throwables) {
//                    JOptionPane.showMessageDialog(null, "Categoryid cannot be found!", "Error!",JOptionPane.ERROR_MESSAGE);
                ...
                sql = "SELECT * FROM device WHERE device.categoryid=?";
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ps.setInt(1, Integer.parseInt(textField2.getText().trim()));
                    try (ResultSet rs = ps.executeQuery()) {
                        if (!rs.next()) {
...
//                    JOptionPane.showMessageDialog(null, "Categoryid cannot be found!", "Error!",JOptionPane.ERROR_MESSAGE);
            ...
            JOptionPane.showMessageDialog(null, "Delete the user information successfully!");
        }
    }
